...
class IPMIPowerCommand extends Command
{
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->prefixProcessor = 'processor_power_';
        $this->processors = config('queue.processor.power');
        $this->user_processor = config('queue.processor.user_power');
    ...
    protected function validSyntax(string $mode, HostService $hostService): bool
    {
        ...
        $this->error('Sai cú pháp. Dùng: php artisan ipmi:power all hoặc php artisan ipmi:power host:<ip>');
        return false;
    }
    ...
    protected function handlerAllSyntax(HostService $hostService) {
        ...
        foreach ($hosts as $index => $host) {
            ...
            if (RedisDispatchCommandService::isExist($host->ip, $this->queue($index))) {
                $this->info("[Tiến trình: ".$this->queue($index)."] Bỏ qua vì đã tồn tại trong Redis cache Job lấy power của host {$host->ip} ");
                continue;
            }

            dispatch(new PowerCrawler($host->ip, $host->username, $host->password, $this->queue($index)))
            ...
            $this->info("[Tiến trình: ".$this->queue($index)."] Đã dispatch PowerCrawler cho {$host->ip}");
        }
    ...
    protected function handlerIpSyntax($mode, HostService $hostService) {
        ...
        if (RedisDispatchCommandService::isExist($ip, $this->user_processor)) {
            $this->info("[Tiến trình: ".$this->user_processor."] Bỏ qua vì đã tồn tại trong Redis cache Job lấy power của host {$host->ip}");
            return;
        }

        dispatch(new PowerCrawler($host->ip, $host->username, $host->password, $this->user_processor))
        ...
        $this->info("[Tiến trình: ".$this->user_processor."] Đã dispatch PowerCrawler cho {$host->ip}");
        return;
    }
    ...
    protected function redisContent($queue, $ip) {
        return [
            'host_ip'=> $ip,
            'queue_worker'=>$queue,
            'dispatch'=> 'Jobs/PowerCrawler.php',
            'note' => 'Job này lấy trạng thái io của host có ipmi',
